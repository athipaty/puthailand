...
export default function AddEntry() {
  ...
  async function submit(e) {
    e.preventDefault();
    if (!form.code) return setError('Select an account');
    if (!form.debit && !form.credit) return setError('Enter debit or credit amount');
  ...
  return (
    <div className="max-w-xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">บันทึกรายการในสมุดรายวัน</p>
      </div>
      ...
      <form onSubmit={submit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Journal (สมุด)</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Voucher No.</label>
            <input value={form.voucher} onChange={e => set('voucher', e.target.value)} placeholder="e.g. PV69030501"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
          </div>
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">Description (คำอธิบาย)</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Debit (เดบิต)</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Credit (เครดิต)</label>
