...
const TYPE_KEYS = ['Asset','Liability','Equity','Revenue','Expense','Other'];
...
export default function ChartOfAccounts() {
  ...
  async function seed() {
    if (!confirm(t('accounts.seedConfirm'))) return;
    setSeeding(true);
    try {
  ...
  const typeFilters = ['All', ...TYPE_KEYS];
  const typeLabel = k => k === 'All' ? t('accounts.all') : t(`accounts.types.${k}`);
  ...
  const grouped = TYPE_KEYS.reduce((acc, type) => {
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{t('accounts.count', { count: accounts.length })}</p>
        </div>
        <div className="flex gap-2">
          {accounts.length === 0 && (
            <button onClick={seed} disabled={seeding}
              ...
              {seeding ? t('accounts.seeding') : t('accounts.seedAccounts')}
            </button>
          )}
      ...
      {showForm && (
        <form onSubmit={addAccount} className="bg-white rounded-xl border border-gray-200 p-5">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">{t('accounts.newAccount')}</h3>
          <div className="grid grid-cols-3 gap-3">
            <input value={form.code} onChange={e => setForm(f=>({...f,code:e.target.value}))}
              required placeholder={t('accounts.codePlaceholder')}
              className="border border-gray-300 rounded-lg px-3 py-2 text-sm" />
            <input value={form.name} onChange={e => setForm(f=>({...f,name:e.target.value}))}
              required placeholder={t('accounts.namePlaceholder')}
              className="border border-gray-300 rounded-lg px-3 py-2 text-sm" />
            <select value={form.type} onChange={e => setForm(f=>({...f,type:e.target.value}))}
              className="border border-gray-300 rounded-lg px-3 py-2 text-sm">
              {TYPE_KEYS.map(k => <option key={k} value={k}>{t(`accounts.types.${k}`)}</option>)}
            </select>
          </div>
          ...
          {typeFilters.map(k => (
            <button key={k} onClick={() => setFilter(k)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                filter === k ? 'bg-slate-800 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'
              }`}>
              {typeLabel(k)}
            </button>
          ))}
      ...
      {/* Grouped table */}
      {loading ? (
        <div className="text-center py-12 text-gray-400">{t('accounts.loading')}</div>
      ) : Object.entries(grouped).map(([type, items]) => (
        <div key={type} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className={`px-4 py-2.5 border-b border-gray-100 ${ACCOUNT_TYPE_COLOR[type]}`}>
            <span className="text-xs font-semibold uppercase tracking-wide">{t(`accounts.types.${type}`)}</span>
            <span className="ml-2 text-xs opacity-60">({items.length})</span>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-gray-400 border-b border-gray-50">
                <th className="px-4 py-2 text-left font-medium w-28">{t('accounts.code')}</th>
                <th className="px-4 py-2 text-left font-medium">{t('accounts.name')}</th>
      ...
      {!loading && visible.length === 0 && (
        <div className="text-center py-16 text-gray-400">
          {accounts.length === 0
            ? <div>{t('accounts.noAccountsYet')}</div>
            : t('accounts.noMatch')
          }
        </div>
      )}
